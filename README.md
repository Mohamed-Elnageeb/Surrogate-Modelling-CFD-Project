...
1. **Convert SU2 outputs into snapshots.** The UNet expects `.npz` files with
   ...
   volume solution and matching surface-force file into that format. Run the
   following Python snippet from your shell (POSIX or Windows) after replacing
   the placeholder paths with your real SU2 outputs:
   ```python
   from pathlib import Path
